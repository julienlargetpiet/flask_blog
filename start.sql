...
CREATE TABLE posts (date_time DATETIME, text_content LONGTEXT, title VARCHAR(255), tags VARCHAR(255), files_name LONGTEXT, allow_comments TINYINT, modified BOOLEAN, username VARCHAR(255));
CREATE TABLE users (username VARCHAR(16), password VARCHAR(162), ip VARCHAR(32), allow_post BOOLEAN, allow_news BOOLEAN, allow_rm_com BOOLEAN, allow_user_ban BOOLEAN, allow_recom BOOLEAN, max_comments_per_day INT UNSIGNED, lst_date INT UNSIGNED);
